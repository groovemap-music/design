#!/usr/bin/env python3
"""Matching unit and the deterministic baseline, copied from gm-design-zwy with the
Semantica encoding removed and the ADR 0014 section 4 comparison vector added.
...
assertion. Nothing here orders candidates either: ``baseline_score`` is a function of
``comparison`` alone, and callers group equal scores rather than breaking ties.
"""
...
def view_of(rec: dict, keep_raw: bool = True) -> View:
    ...
    if not keep_raw:
        # Only diagnostics read these; dropping them roughly halves a large pool's
        # footprint.
...
# The fields ADR 0014 section 4 names for ambiguity, in comparison-vector order.
COMPARED = ("barcode", "label_catno", "title", "artist", "year", "format", "country")
_WEIGHTS = {"barcode": (0, 8), "label_catno": (0, 2, 4), "title": (0, 1, 3), "artist": (0, 3),
            "year": (0, 1), "format": (0, 1), "country": (0, 1)}


def comparison(q: View, c: View, drop: frozenset[str] = frozenset()) -> tuple[int, ...]:
    """How a candidate compares with the query on each ADR 0014 section 4 field, as a
    level per field (0 = no agreement). Two candidates with the same vector are equal on
    every field the query carries and the matcher compares, which is the section 4
    definition of an ambiguous group; equal scores alone are not."""
    barcode = int("barcode" not in drop and bool(q.barcodes & c.barcodes))
    if "catno" in drop:
        label_catno = 0
    elif q.label_catnos & c.label_catnos:
        label_catno = 2
    else:
        label_catno = int(bool(q.catnos & c.catnos))
    if "title" in drop:
        title = 0
    elif q.title and q.title == c.title:
        title = 2
    else:
        title = int(_token_jaccard(q.title, c.title) >= 0.5)
    artist = int("artist" not in drop and bool(q.artists & c.artists))
    desc = "descriptors" not in drop
    year = int(desc and bool(q.year) and q.year == c.year)
    fmt = int(desc and bool(q.formats & c.formats))
    country = int(desc and bool(q.countries & c.countries))
    return (barcode, label_catno, title, artist, year, fmt, country)


def score_of(vec: tuple[int, ...]) -> float:
    return float(sum(_WEIGHTS[f][level] for f, level in zip(COMPARED, vec)))
...
def baseline_score(q: View, c: View, drop: frozenset[str] = frozenset()) -> float:
    ...
    name) and were not fitted to the test split. Identical to gm-design-zwy's."""
    return score_of(comparison(q, c, drop))
