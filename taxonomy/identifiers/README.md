...
[`v1/identifier-types.json`](v1/identifier-types.json) is the closed identifier vocabulary that [ADR 0011](../../docs/adr/0011-catalog-identifiers-and-manufacturing-credits.md) makes authoritative for the `identifiers` block a Discogs or MusicBrainz releases event carries. It names the seven canonical types, the raw Discogs and MusicBrainz strings each type maps from, the three alias namespaces that mint `provider_aliases` rows under [ADR 0009](../../docs/adr/0009-native-identity-and-provider-aliases.md), and the normalization each namespace applies before a value becomes an `external_id`.
...
- The first six are the targets the raw Discogs `identifiers` strings map onto. `catalog_number` has no raw Discogs identifier string behind it: it is lifted from the catalogue number the release's label entries carry, so every namespace that mints an alias has an entry in one list.
- MusicBrainz carries no free-form typed identifiers list: a release resource exposes `barcode` and, per `label-info` entry, `catalog-number` as dedicated fields rather than a list of `{type, value}` pairs. Both map onto the same `barcode` and `catalog_number` canonical types Discogs uses, keyed by the MusicBrainz field name itself in `identifier-types.json`'s `musicbrainz.types`.
...
- A MusicBrainz release's `barcode` field contributes a `barcode` item when it is non-empty after trimming; MusicBrainz represents an absent barcode as `null`, not a sentinel string, so a missing or blank value is skipped. Each `label-info` entry contributes a `catalog_number` item the same way Discogs' `labels[].catno` does, from its `catalog-number` field. Both source fields carry `source.type: null` and are recorded under `source.field` as `barcode` and `label-info[].catalog-number` respectively — MusicBrainz has no analogue of `absent_catalog_number`.
...
The MusicBrainz field names are not this organization's design either: `barcode` and `label-info[].catalog-number` are the field names MusicBrainz's own Release JSON representation uses on the release resource at `https://musicbrainz.org/ws/2/release/{mbid}`, documented at the [MusicBrainz API examples and search response reference](https://musicbrainz.org/doc/MusicBrainz_API). Unlike Discogs' `identifiers` array, a MusicBrainz release exposes no typed list of identifiers: `barcode` is a single scalar field (`null` when absent) and each `label-info` entry carries at most one `catalog-number`, so the vocabulary keys `musicbrainz.types` by those two field names rather than by an upstream `type` string.

## Vendoring rule

The vocabulary is vendored verbatim, byte for byte, into every repository that computes or reads the identifiers block, beginning with `discogs-ingestion`, `musicbrainz-ingestion`, and `python-libraries`. A vendored copy sits beside a source record naming the design commit and the SHA-256 of this file, and each repository's check gate fails when the copy drifts from that record. Compute the digest from the exact bytes of the file:
